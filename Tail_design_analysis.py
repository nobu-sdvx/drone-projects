...
# 尾翼基準点
X_TAIL_END    = 0.305     # 機体最後端 [m] (= 尾翼力の作用点)
ELL_H_DESIGN  = X_TAIL_END - X_AC_DESIGN   # 設計点 ℓh [m] = 158.1 mm
ELL_H_LIMIT   = X_TAIL_END - X_AC_LIMIT    # 限界点 ℓh [m] = 154.6 mm
...
def cantilever_root_bending_moment(total_force, span):
    """
    等分布荷重を受ける片持ち梁の根元曲げモーメント
      w = F / b                            (単位長さあたり荷重)
      M_root = w · (b/2)² / 2             (半翼長 b/2 にわたる積分)

    Parameters
    ----------
    total_force : 尾翼にかかる合力 [N]
    span        : 翼幅 [m]
    """
    half_span = span / 2.0
    w = total_force / span
    M_root = w * half_span ** 2 / 2.0
    return M_root
...
def compute_tail_structure(L_worst, span, chord, spar_diameter_mm=1.0, label=''):
    """
    ...
    """
    M_root = cantilever_root_bending_moment(L_worst, span)
    ...
    print(f"    翼根曲げモーメント M_root = w·(b/2)²/2 = {M_root*1000:.3f} mN·m")
    print(f"")
...
def main():
    ...
    L_t_d, M_ac_d, M_lever_d = trim_tail_force(
        ...
        X_CG_DESIGN, X_AC_DESIGN, X_TAIL_END,
    )
    arm_d = X_TAIL_END - X_CG_DESIGN
    print(f"\n  [設計点 CG@MAC35%]")
    ...
    L_t_l, M_ac_l, M_lever_l = trim_tail_force(
        CM_WING, q, S_WING, C_WING, W_force,
        X_CG_LIMIT, X_AC_LIMIT, X_TAIL_END,
    )
    arm_l = X_TAIL_END - X_CG_LIMIT
    print(f"\n  [限界点 CG@MAC30%]")
    ...
    struct_h = compute_tail_structure(
        ...
        label='水平尾翼 (最悪荷重 L_worst を集中で受ける想定)',
    )

    # 垂直尾翼: トリム時の側力は通常極小だが、保守的にH-tail最悪荷重と同等を仮定
    print(f"\n  [垂直尾翼の強度確認]")
    print(f"    トリム時は側力ほぼゼロ。突風・操舵で発生する側力の最悪値を")
    print(f"    保守的に H-tail と同じ L_worst で評価。翼幅は短く翼弦は近いので")
    print(f"    モーメントアームが小さく、応力は H-tail より低くなる。")
    struct_v = compute_tail_structure(
        L_worst=L_t_worst,
        span=bv, chord=cv,
        spar_diameter_mm=1.0,
        label='垂直尾翼 (保守的に H-tail と同等荷重)',
    )
...
def plot_aircraft_layout(results, save_path='plots/tail_layout_2026-05-07.png'):
    ...
    # ===== 上面図 =====
    # 胴体ライン
    ax_top.plot([0, X_TAIL_END * 1000], [0, 0], 'k-', linewidth=1.0)
    # POWERUP
    ...
    # 水平尾翼
    h_tail_le = X_TAIL_END * 1000 - results['ch_mm']
    ...
    # ℓh アロー
    ax_top.annotate('', xy=(X_TAIL_END * 1000, 100), xytext=(X_AC_DESIGN * 1000, 100),
                    arrowprops=dict(arrowstyle='<->', color='red', linewidth=1.5))
    ax_top.text((X_AC_DESIGN + X_TAIL_END) / 2 * 1000, 110,
    ...
    # 尾翼ブーム
    ax_side.plot([220, X_TAIL_END * 1000], [0, 0], 'k-', linewidth=2.5, label='Tail boom')
    # 水平尾翼 (側面では薄く)
    h_tail_le = X_TAIL_END * 1000 - results['ch_mm']
    ...
    # 垂直尾翼
    v_tail_le = X_TAIL_END * 1000 - results['cv_mm']
